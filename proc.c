...
exit(int status)
{
  ...
  curproc->exitStat = status;   //added for Lab 1 part a
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        p->state = UNUSED;
        release(&ptable.lock);
        //added for Lab 1 part b
        if(status){               //!null
          *status = p->exitStat;    //pass the status of the child the got terminated
        }
        return pid;
      }
    }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      temp = p;
      for(p_1 = ptable.proc; p_1 < &ptable.proc[NPROC]; p_1++){
        if (p_1->state != RUNNABLE)
          continue;
        if (temp->priority_value > p_1->priority_value)
          temp = p_1;
      }
...
getpriority(void)
{
  return (int) myproc()->priority_value;
}
